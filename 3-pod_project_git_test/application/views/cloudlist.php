					...
					if(action=='start'){ 
						$.ajax({
								...
								success : function(data){
									 if(data.jobid){
									 	async(data.jobid,'SERVER REBOOT');
									 }else{
										 setModalMsg('SERVER 시작 실패').modal(); 
									 } 
								},
								error : function( ){  
									 setModalMsg('SERVER 시작 실패(error)').modal(); 
								}
						}); 
					}else if(action == 'stop'){ 
						$.ajax({
							...
							success : function(data){
								 if(data.jobid){
								 	async(data.jobid,'SERVER STOP');
								 }else{
									 setModalMsg('SERVER 정지 실패').modal(); 
								 } 
							},
							error : function( ){  
								setModalMsg('SERVER 정지 실패(error)').modal(); 
							}
						}); 
					}else if(action == 'reboot'){ 
						$.ajax({
							type : "POST",
							url: '/cloudlist/rebootVM',
							data : formData,
							dataType: 'json',
							success : function(data){
								 if(data.jobid){
									...
									setModalMsg('SERVER 재부팅 실패').modal(); 
								 } 
							},
							error : function( ){  
								setModalMsg('SERVER 재부팅 실패(error)').modal(); 
							}
					}); 
				}  
		});

		$('#osinitializebtn').click(//asdf
				function(){
					vmid = $('#serverinfo_table #vmid').html(); 
...
// 										 showObJ(data.jobresult);
										 if(data === 'error'){ 
											 setModalMsg('OS 초기화 실패(asnc)').modal();
										 }else{
											 vm = data.jobresult.virtualmachine;
											 setModalMsg("<span style='color:blue'>"+vm.displayname+'</span>서버 OS 초기화 완료').modal();
										 }
									 },
									 error:function(){ 
										 setModalMsg('OS 초기화 실패(error,async)').modal();
									 },
									 complete:function(){
								...
								setModalMsg('OS 초기화 실패').modal();
							}
						},
						error : function( ){ 
							setModalMsg('OS 초기화 실패(error)').modal();
						}
					}); 
				}
		);
...
// 										 showObj(data);
										 if(data === 'error'){ 
											 setModalMsg('비밀번호 초기화 실패(asnc)').modal();
										 }else{
											 vm = data.jobresult.virtualmachine;
											 password = vm.password;
											 vmname = vm.displayname;
											 setModalMsg("클라우드 서버 비밀번호 변경 완료<br><span style='color:blue'>"+vmname+"</span>의 비밀번호는 <span style='color:red'>"+password+'</span>입니다').modal();
										 }
									 },
									 error:function(){ 
										 setModalMsg('비밀번호 초기화 실패(error,async)').modal();
									 },
									 complete:function(){
										 $('#loadingModal').modal('hide'); 
									 }
								...
								setModalMsg('비밀번호초기화 실패').modal();
							}
						},
						error : function( ){ 
							setModalMsg('비밀번호초기화 실패(error)').modal();
						}
					});
				}
		); 
		...
		$('#deletevmbtn').click(
				function(){
					vmid = $('#serverinfo_table #vmid').html(); //asdf
					$.ajax({
						type:'GET',
						...
						success : function(data){
							 if(data.jobid){ 
							 	async(data.jobid,'SERVER 삭제');
							 }else{
								setModalMsg('서버 삭제 실패').modal();
							 } 
						},
						error : function( ){ 
							setModalMsg('서버 삭제 실패(error)').modal(); 
						}
					});
				}
		); 
...
<!-- 서버목록 -->
	<div class="container-fluid">
		...
		for($i = 0; $i < $vmcount; $i ++) {
			...
			$vmstate = $vm['state'];
			echo $vm ['state'];
			if($vmstate == 'Stopped' || $vmstate == 'Running'){
			}else{
				echo "<img src='/redcircleloading.gif' style='width:20px'>";
			}
