<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class Volumelist extends MY_Controller{
		...
		function getVolumesByZoneid($zoneid){
			 $result = $this->volumesModel->getVolumesByZoneid($zoneid);
			 return $result;
		}
		...
		function searchVolumes($zoneid, $searchword){
			if($zoneid == 'all'){
				...
				$listVolumes = $this->volumesModel->getVolumesByZoneid($zoneid);
			}
// 			echo var_dump($listVolumes);
		...
		function showSearchResult($zoneid, $searchword){
			...
			$volumes = $this->searchVolumsForArray($zoneid,$searchword); //무조건 array생성해서들어와
